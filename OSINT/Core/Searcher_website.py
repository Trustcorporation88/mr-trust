...
class Web:
    ...
    def search(username, Mode):
        ...
        try:
            domain_result = buscar_dominio(username)
            domain_report_path = salvar_relatorio_dominio(domain_result)
            f = open(report, "a", encoding="utf-8")
            f.write("\nDOMAIN INTELLIGENCE REPORT:\r\n")
            f.write("PATH: {}\r\n".format(domain_report_path))
            f.write("IP: {}\r\n".format(domain_result.get("ip", "N/A")))
            f.write("COUNTRY: {}\r\n".format(domain_result.get("geo", {}).get("country", "N/A")))
            f.write("CITY: {}\r\n".format(domain_result.get("geo", {}).get("city", "N/A")))
            f.write("HOSTING: {}\r\n".format(domain_result.get("hosting", {}).get("isp", "N/A")))
            f.write("SSL: {}\r\n".format("YES" if domain_result.get("ssl", {}).get("available") else "NO"))
            f.write("SUBDOMAINS: {}\r\n".format(len(domain_result.get("subdomains", {}).get("found", []))))
            f.close()
            print(Font.Color.GREEN + "\n[+]" + Font.Color.WHITE + " DOMAIN REPORT: {}".format(domain_report_path))
        except Exception as e:
            print(Font.Color.RED + "\n[!]" + Font.Color.WHITE + " DOMAIN SEARCH ERROR: {}".format(str(e)[:120]))
