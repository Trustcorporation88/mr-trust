...
st.set_page_config(
    page_title="MR TRUST OSINT - Dashboard de Inteligência",
    page_icon="🔍",
    layout="wide",
...
def render_catalog_row(items, key_prefix: str = "catalog"):
    ...
    for idx, (col, item) in enumerate(zip(cols, items)):
        ...
        with col:
            ...
            is_current_page = item['key'] == st.session_state.get("page", "home")
            
            button_disabled = is_current_page
            button_type = "secondary" if is_current_page else "primary"
            ...
            if st.button(
                ...
                # Atualiza a página no estado da sessão
                st.session_state["page"] = item["key"]
                
                # Adiciona suporte a prefill se necessário
...
def main():
    """
    Ponto principal de execução do dashboard OSINT do Mr. Holmes
    """
    inject_theme()
    ...
    # Gerenciamento de navegação segura
    if st.session_state["page"] == "home":
        ...
        st.warning("Módulo de Grafo em desenvolvimento")
    
    # Renderiza trilhas relacionadas para navegação contextual
